...
namespace Cyclops.MultiCluster.Controllers
{
    ...
    [EntityRbac(typeof(V1VeccGslb), Verbs = RbacVerb.Get | RbacVerb.List | RbacVerb.Watch)]
    public class K8sChangedController :
        IEntityController<V1Ingress>,
        IEntityController<V1Service>,
        IEntityController<V1EndpointSlice>,
        IEntityController<V1Gslb>,
        IEntityController<V1VeccGslb>
    {
        ...
        /// <param name="cancellationToken"></param>
        /// <returns></returns>
        public async Task<ReconciliationResult<V1VeccGslb>> DeletedAsync(V1VeccGslb gslb, CancellationToken cancellationToken)
        {
            _logger.LogInformation("VeccGSLB deleted {@namespace}/{@name}", gslb.Namespace(), gslb.Name());
            using var _scope = _logger.BeginScope(new { originalResourceType = "veccgslb" });

            var cyclopsGslb = new V1Gslb
            {
                Metadata = gslb.Metadata,
                Spec = new V1Gslb.GslbSpec
                {
                    Hostnames = gslb.Hostnames,
                    IPOverrides = gslb.IPOverrides,
                    ObjectReference = new V1Gslb.V1ObjectReference
                    {
                        Kind = gslb.ObjectReference.Kind == V1VeccGslb.V1ObjectReference.ReferenceType.Ingress
                            ? V1Gslb.V1ObjectReference.ReferenceType.Ingress
                            : V1Gslb.V1ObjectReference.ReferenceType.Service,
                        Name = gslb.ObjectReference.Name
                    },
                    Priority = gslb.Priority,
                    Weight = gslb.Weight
                }
            };

            await DeletedAsync(cyclopsGslb, cancellationToken);
            return ReconciliationResult<V1VeccGslb>.Success(gslb);
        }
        ...
        /// <param name="cancellationToken"></param>
        /// <returns></returns>
        public async Task<ReconciliationResult<V1VeccGslb>> ReconcileAsync(V1VeccGslb gslb, CancellationToken cancellationToken)
        {
            _logger.LogInformation("Reconciling V1VeccGslb {@namespace}/{@name}", gslb.Namespace(), gslb.Name());
            using var _scope = _logger.BeginScope(new { originalResourceType = "veccgslb" });
            ...
            await ReconcileAsync(cyclopsGslb, cancellationToken);
            return ReconciliationResult<V1VeccGslb>.Success(gslb);
        }
        ...
        public async Task<ReconciliationResult<V1Gslb>> DeletedAsync(V1Gslb gslb, CancellationToken cancellationToken)
        {
            ...
            if (spec.ObjectReference.Kind == V1Gslb.V1ObjectReference.ReferenceType.Ingress)
            {
                _logger.LogInformation("GSLB has ingress references, syncing");
                var ingress = await _client.GetAsync<V1Ingress>(spec.ObjectReference.Name, gslb.Namespace());
                if (ingress == null)
                {
            ...
            else
            {
                ...
                var service = await _client.GetAsync<V1Service>(spec.ObjectReference.Name, gslb.Namespace());
                if (service == null)
                {
        ...
        public async Task<ReconciliationResult<V1Gslb>> ReconcileAsync(V1Gslb gslb, CancellationToken cancellationToken)
        {
            ...
            if (gslb.Spec.ObjectReference.Kind == V1Gslb.V1ObjectReference.ReferenceType.Ingress)
            {
                _logger.LogDebug("GSLB has ingress references, syncing");
                var ingress = await _client.GetAsync<V1Ingress>(gslb.Spec.ObjectReference.Name, gslb.Namespace());
                if (ingress == null)
                {
                    _logger.LogInformation("Ingress not found, resyncing entire cluster");
                    await _synchronizer.SynchronizeLocalClusterAsync();
            ...
            {
                _logger.LogInformation("GSLB has service references, syncing");
                var service = await _client.GetAsync<V1Service>(gslb.Spec.ObjectReference.Name, gslb.Namespace());
                if (service == null)
                {
