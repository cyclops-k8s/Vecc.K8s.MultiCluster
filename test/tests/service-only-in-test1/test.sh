...
setup() {
    ...
    set_namespace service-only-in-test1
    (( RETCODE+=$? )) || true
    ...
    wait_for_ips "service.only-in-test1.test1" $CLUSTER1IP
    (( RETCODE+=$? )) || true
...
cleanup() {
    kubectl delete namespace service-only-in-test1
    return $?
}